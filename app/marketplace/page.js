...
function getCompositeScore(solution) {
  ...
  return communityScore.average; // Early signal - just community
}
  ...
  const filteredAndSortedSolutions = useMemo(() => {
    let filtered = solutions.filter((solution) => {
      ...
      for (const [category, selectedValues] of Object.entries(selectedTags)) {
        if (selectedValues.length > 0) {
          const solutionTags = solution.tags[category] || [];
    ...
    switch (sortBy) {
      ...
      case 'community-score':
        filtered = filtered.sort((a, b) => b.communityScore.average - a.communityScore.average);
        break;
      case 'ryan-score':
        filtered = filtered.sort((a, b) => b.ryanScore.rating - a.ryanScore.rating);
        break;
      case 'most-reviewed':
        filtered = filtered.sort((a, b) => b.communityScore.count - a.communityScore.count);
        break;
      case 'newest':
      ...
      <main className="flex-1 px-4 sm:px-8 lg:px-16 py-8">
        ...
        <div className="max-w-7xl mx-auto mb-8">
          ...
          <p className="text-lg text-white/70 max-w-3xl">
            A curated directory of solutions for manufacturing transformation. No pay-to-play, 
            no affiliate links—just honest recommendations based on what actually works in real plants.
          </p>
        </div>
          ...
          <div className="lg:hidden">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="px-2 py-0.5 text-xs rounded-full bg-blue-500">{activeFilterCount}</span>
                )}
              </span>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex-1">
            ...
            <div className="grid gap-6">
              {filteredAndSortedSolutions.map((solution) => {
                ...
                return (
                  <Link key={solution.id} href={`/marketplace/${solution.slug}`} className="group">
                    <div
                    ...
                    >
                      <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-4 mb-2">
                            ...
                            <div className="flex flex-col items-end flex-shrink-0">
                              <div className="flex items-center gap-1 text-yellow-400">
                                ...
                                <span className="text-sm font-medium">{compositeScore.toFixed(1)}</span>
                              </div>
                              <span className="text-white/40 text-xs">
                                {scoreLabel || `${solution.communityScore.count} reviews`}
                              </span>
                            </div>
                          </div>

                          <p className="text-white/70 text-sm mb-3 line-clamp-2">{solution.summary}</p>

                          {/* Values lens tags */}
                          <div className="flex flex-wrap gap-2 mb-3">
                            {solution.tags.valuesLens?.slice(0, 3).map((tag) => (
                              <span
                                key={tag}
                          ...
                          {/* Tech category tags */}
                          <div className="flex flex-wrap gap-2">
                            {solution.tags.techCategory.slice(0, 3).map((tag) => (
                              <span
                                key={tag}
